...
- **함께 모드 (1:1)** — Nearby Connections로 직접 연결된 두 기기가 실시간 동기화. 한쪽이 사진을 보내면 양쪽 모두 같은 사진 위에 함께 그림.
...
함께 모드 검증은 **에뮬레이터로 불가** — 실제 안드로이드 기기 2대 필요 (둘 다 Google Play Services 탑재).
...
함께 모드 진입 시점에만 요청. 싱글 모드는 카메라 촬영 시점에만 카메라 권한.
