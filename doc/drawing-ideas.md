# 그리기 모드 강화 아이디어

> 그리기 경험을 풍부하게 만들기 위한 기능 후보 목록. 사용자 제안 + 구현 관점 평가.
> 상태: 1~4(안내선·에어브러시·번짐·스티커) 완료. 나머지는 백로그.

## 평가 기준 (이 앱 특유)

- **동기화**: 모임/함께 모드에서 모든 그리기 요소는 `DrawingEvent` → `CanvasState.apply()`
  경로로 동기화돼야 함. 새 요소가 stroke 틀에 맞으면 거의 공짜, 아니면 와이어(`Frame`)
  까지 새로 만들어야 함.
- **렌더 공유**: `ui/canvas/StrokeRenderer.kt#drawStroke` 가 화면 + PNG 합성 공유. 여기에
  그리면 "보이는 것 = 저장되는 것" + 멀티 동기화가 자동.
- **정규화 좌표**: 모든 점은 0..1. 렌더 시 `canvasSize` 곱함.

## 완료된 항목

구현 세부는 코드와 [roadmap.md](roadmap.md) Phase 5.5 참고. 여기선 요약만.

1. **안내선(가이드라인)** — 중앙 십자선 + 격자(6×6 / 18×18 택1). 로컬 전용(동기화·저장
   미포함). `DrawingViewModel.guideCross/guideGrid`, `DrawingCanvas.drawGuides`, 도구바 안내선 아이콘.
2. **에어브러시(분사)** — `BrushType.Airbrush`. 분사점을 stroke 에 저장하지 않고 렌더 시
   결정론 seed(`stroke.id` 해시)로 생성 → 매 프레임·양 단말 동일. `StrokeRenderer.drawAirbrush`.
3. **번짐(수채/스머지)** — `BrushType.Blur`. native `Paint` + `BlurMaskFilter` via
   `drawIntoCanvas { nativeCanvas.drawPath }`. PNG 합성에서도 동작. `StrokeRenderer.drawBlurred`.
4. **스티커** — 자체 벡터 12종(하트/별/스마일/꽃/구름/해/달/무지개/물방울/번개/보석/반짝이)
   배치·이동·크기·회전·삭제 + stroke/스티커 통합 undo(`UndoItem`). 메타(`key`+변환)만 보관·전송,
   렌더 시 벡터로 치환. 변형은 commit-on-end. 상세: [sticker-plan.md](sticker-plan.md).

> **성능 메모**: 에어브러시·번짐은 stroke 가 많이 쌓이면 매 프레임 재계산/blur 비용이 큼 →
> "완료 stroke 비트맵 캐시"(아래 백로그 §구조적) 필요성이 커짐. 실측 후 도입.

## 백로그 (우선순위순)

> **정렬 기준**: 우선순위 = (가치 × 구조 적합성) ÷ 노력.
> - **가치**: 어린이/일반 사용자 재미·실용. **구조 적합성**: stroke·렌더·오버레이 등 기존
>   틀에 얹히면 ↑, 와이어(`Frame`)·구조 변경/정책 위험이면 ↓. **노력**: 난이도.
> - 그래서 "기존 brush/오버레이 재사용 + 로컬 전용"이 위로, "구조 변경·정책 위험"이 아래로.
>
> | 항목 | 가치 | 구조 적합성 | 노력 | 티어 |
> |---|---|---|---|---|
> | 브러시 변형 3종 | 중상 | 높음(자동 동기화) | 낮 | P1 |
> | 속도 기반 굵기 | 중 | 높음(렌더 유도) | 낮 | P1 |
> | 손떨림 보정 | 상(실용) | 높음(로컬·자동) | 낮~중 | P1 |
> | 스포이드 | 상(사진앱) | 무관(로컬) | 낮~중 | P1 |
> | 최근 색/팔레트 | 중 | 무관(로컬) | 낮 | P1 |
> | 트레이싱(반투명) | 상(정체성) | 무관(로컬) | 낮 | P2 |
> | 대칭(미러) | 중 | 높음(자동 동기화) | 중 | P2 |
> | 도형 채우기 토글 | 중 | 높음 | 낮 | P2 |
> | 배경색 선택 | 중 | 낮(상태 추가) | 낮 | P2 |
> | redo | 중 | 중(undo 확장) | 중 | P3 |
> | 타임랩스 재생 | 상(공유성) | 중(로그 정책) | 중 | P3 |
> | 트레이싱(엣지검출) | 중 | 무관(로컬) | 중 | P3 |
> | 비트맵 캐시 | 조건부(성능) | — | 중 | P3 |
> | 레이어/텍스트 | 상 | 낮(큰 구조) | 상 | P4 |
> | 음향 드로잉 | 불확실 | 중 | 중상+정책 | P4 |

## P1 — 즉시 (가볍고 가치 큼, 기존 틀 재사용)

### 브러시 변형 3종 (네온/점선/무지개)
기존 `BrushType` enum + `StrokeRenderer` 분기 추가만으로 화면·PNG·멀티 동기화 자동 공유.
가장 가벼운 묶음.
- **네온/glow**: 발광 효과. `BlurMaskFilter`(번짐) + 밝은 색 겹침 응용 — 거의 공짜.
  어두운 배경에서 화려.
- **점선/패턴**: `PathEffect.dashPathEffect` 로 파선. 한 줄 수준.
- **무지개**: stroke 진행에 따라 색 변화. 에어브러시처럼 결정론(`stroke.id` seed)으로
  좌표→색 매핑. 어린이 인기.
- **난이도**: 낮음. 3종 묶어 한 번에 권장.

### 속도 기반 굵기 (붓펜/캘리그래피)
- **개념**: 손가락 이동 속도(점 간 거리)로 선 굵기 변조 — 빠르면 가늘게, 느리면 굵게.
  붓펜·캘리그래피 느낌. 어린이보다는 "예쁘게 쓰기" 욕구에 호응.
- **구현**: `Point` 에 이미 좌표 있음 → 점 간 거리로 굵기 결정론 계산, `StrokeRenderer`
  분기 추가. 와이어 변경 불필요(굵기는 렌더 시 점에서 유도). 브러시 변형 묶음에 끼우기 좋음.
- **난이도**: 낮음. 브러시 변형 3종과 함께 처리 권장.

### 손떨림 보정 (스트로크 안정화)
- **개념**: 입력 점을 이동평균/스플라인으로 부드럽게 → 떨리는 선을 매끈하게. 터치·어린이
  사용자에게 특히 실용적.
- **구현**: `DrawingCanvas` 입력 단계에서 점 평활화 후 emit. 정규화 좌표 그대로, 보정된
  점만 stroke 에 저장·전송하므로 동기화·저장·undo 자동. 강도 토글 UI(약/강).
- **난이도**: 낮~중. 평활화 알고리즘 선택(지수이동평균이 가장 단순)이 전부. 가성비 높음.

### 스포이드 (색 추출)
- **개념**: 캔버스/사진 배경에서 색을 집어 현재 펜 색으로. **사진 위 그리기 앱이라 특히
  실용적.**
- **구현**: 탭 지점의 픽셀 색 읽기(렌더된 비트맵 또는 배경 bitmap 샘플). `ToolKind.Eyedropper`
  또는 색 팔레트의 스포이드 버튼. 동기화 무관(로컬 색 선택일 뿐).
- **난이도**: 낮~중. 캔버스 픽셀 캡처(`Picture`/`drawToBitmap`) 또는 배경 직접 샘플.

### 최근 색 / 색 팔레트 저장
- **개념**: 최근 사용 색 표시 + 즐겨찾기 팔레트 저장. 스포이드와 짝 — 추출한 색을 보관.
- **구현**: 로컬 상태(최근 N색)·간단 영속(`SharedPreferences` 수준)만. 캔버스/동기화 무관.
- **난이도**: 낮음. 스포이드와 함께 처리하면 자연스러움.

## P2 — 그다음 (정체성·재미, 약간 더 노력)

### 트레이싱 보조 (사진 따라 그리기)
- **개념**: 사진 배경을 반투명화하거나 외곽선만 추출해 오버레이 → 그 위를 따라 그림.
  **"사진 위 그리기" 라는 앱 정체성을 가장 직접적으로 강화.**
- **구현**: 반투명은 배경 알파 조절만(매우 가벼움 — **이 부분이 P2**). 외곽선 추출은 엣지 검출
  (Sobel/Canny 근사) 후 라인 오버레이로 노력이 커 **엣지 부분은 P3**. 로컬 전용, 동기화 무관
  (배경 표시 옵션일 뿐). 안내선처럼 저장·전송 미포함.
- **난이도**: 반투명 낮음 / 엣지 검출 중. 반투명부터 단계적 도입 권장.

### 대칭(미러) 그리기
- **개념**: 좌우/상하(또는 N분할 만다라) 대칭으로 stroke 자동 복제. 안내선(중앙 십자선)과
  시너지, 어린이/낙서 재미.
- **구현**: 입력 stroke 의 미러 좌표 stroke 를 같이 생성·emit → 동기화·undo·저장 자동
  (정규화 좌표 반사). 대칭 축 토글 UI.
- **난이도**: 중간. 입력 시 미러 이벤트 동반 생성이 핵심.

### 도형 채우기 토글
- **개념**: 현재 도형은 외곽선만 → 채움 옵션. 작지만 자주 쓰는 옵션.
- **구현**: `ToolSettings` 에 fill 플래그 추가, `StrokeRenderer` 도형 분기에서 채움 적용 →
  화면·PNG·동기화 자동. **난이도**: 낮음.

### 배경색 선택
- **개념**: 흰 배경 대신 파스텔 등 선택. 사진 없을 때 분위기.
- **구현**: 캔버스 배경색 상태 추가(현재 배경은 사진 슬롯뿐). 간단하지만 새 상태/저장 반영
  필요. **난이도**: 낮음.

## P3 — 큰/조건부 (구조 변경·성능, 먼저 검토 필요)

### redo
- 현재 undo 만. undo 스택은 스티커 도입 때 `UndoItem` 으로 일반화됨 — redo 스택을 그 위에 얹기.
  **난이도**: 중(되돌린 항목 보관·재적용 + 멀티 동기화 시 redo 의미 정리).

### 타임랩스 재생·공유
- **개념**: 그리는 과정을 기록해 "재생"·GIF/영상으로 공유. 완성작뿐 아니라 *과정*을
  보여줘 SNS 친화적. 함께 모드면 양쪽 합작 과정이 그대로 재생됨.
- **구현**: **이벤트 소싱 구조와 시너지 큼** — 모든 변경이 이미 `DrawingEvent` → `apply()`
  를 거치니, 이벤트 순서(+상대 타임스탬프)만 로그로 보관하면 재생은 거의 공짜. 재생은
  로그를 스케줄러로 `apply()` 재실행. 영상 인코딩은 별도 작업(프레임 캡처 → MediaCodec).
- **난이도**: 중. + **정책 메모**: 이벤트 로그 보관 위치·용량 정책을 정해야 함(메모리 누적
  vs 디스크 스풀) → 약간 구조적. 재생만 먼저, 영상 내보내기는 후속으로 분리 권장.

### 완료 stroke 비트맵 캐시 (조건부 — 실측 후)
- 에어브러시·번짐 누적 시 매 프레임 재계산/blur 비용이 큼 → 프레임 드랍 대비 성능 작업.
  위 [성능 메모](#완료된-항목) 참고. **실측으로 드랍 확인 후** 도입. **난이도**: 중.

## P4 — 보류 / 실험 (규모 큼·불확실)

### 레이어 / 텍스트 넣기
- 큰 작업, 보류급. 레이어는 렌더·합성·undo·동기화 전반에 영향, 텍스트는 입력/폰트/편집 UI 필요.

### 음향 드로잉 / 비주얼라이저 — 실험 후보 (불확실)
- **개념**: 마이크/음악의 진폭·주파수(FFT)·박자를 받아 stroke 를 자동 생성. 음에 맞춰
  "자연스럽게" 그려지는 재미 요소. 함께 모드면 생성 stroke 가 `DrawingEvent` 로 양쪽 동기화.
- **결과물 성격**: 본질적으로 **추상 비주얼라이저/패턴**. 날것 매핑이면 파형 선 하나,
  매핑을 설계하면(음표→점/별, 음높이→위치·색, 박자→간격) 점묘화처럼 풍부해짐. 단
  "곡의 *의미*(가사/제목)를 형상화" 하는 건 불가 — 그건 AI 이미지 생성 영역.
- **구현**: `AudioRecord`/`Visualizer` 로 실시간 진폭·FFT → stroke 매핑. 매핑 디자인이
  핵심이자 불확실성(여러 번 튜닝 필요). 스티커 기능과 연계하면 음표→별 점묘 가능.
- **난이도**: 중상. + **출시 정책 주의**:
  - `RECORD_AUDIO` 권한 — 현재 앱의 "무권한" 정책이 깨짐(거부감).
  - **음향을 저장·전송하지 않고 실시간 시각화만** 하면 데이터 수집 최소 → 심사 단순.
    생성된 stroke 만 동기화, 오디오 자체는 버림.
  - Play Store: 마이크 권한 자체는 리젝 사유 아님. 단 **개인정보처리방침 + Data safety
    폼 + 권한–기능 일치 + 포그라운드 사용** 필수. 어린이 타겟(Families)이면 마이크 추가
    제약. 가장 흔한 함정은 기술이 아니라 정책 문서 누락.
- **권장**: "마이크 진폭 → 파형 선 하나" 작은 프로토타입으로 느낌부터 확인 후 판단.

## 권장 진행 순서

(완료) 안내선 → 에어브러시 → 번짐 → 스티커.

이제 위 **P1 → P2 → P3 → P4** 티어 순서가 곧 권장 진행 순서. 요약:
- **P1**: 브러시 변형 3종+속도 굵기 → 손떨림 보정 → 스포이드+최근 색 (가장 가벼움·고가치)
- **P2**: 트레이싱(반투명) → 대칭 → 도형 채우기 → 배경색 (정체성·재미·작은 구조)
- **P3**: redo → 타임랩스 재생 → 트레이싱(엣지) → 비트맵 캐시(실측 후) (구조·성능)
- **P4**: 레이어/텍스트 → 음향 드로잉 (규모 큼·정책 불확실)

원칙: 기존 brush/오버레이 재사용 + 로컬 전용은 위로, 와이어·구조 변경·정책 위험은 아래로.
