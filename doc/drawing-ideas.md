...
> | 항목 | 가치 | 구조 적합성 | 노력 | 티어 |
> |---|---|---|---|---|
> | ~~트레이싱(반투명)~~ ✅ | 상(정체성) | 무관(로컬) | 낮 | 완료 |
> | 대칭(미러) | 중 | 높음(자동 동기화) | 중 | P2 |
> | 도형 채우기 토글 | 중 | 높음 | 낮 | P2 |
> | 배경색 선택 | 중 | 낮(상태 추가) | 낮 | P2 |
...
> ~~브러시 변형 3종 + 속도 기반 굵기~~ → **완료** (네온/점선/무지개/붓펜, 위 [완료된 항목](#완료된-항목) 5번 참고).
> ~~손떨림 보정~~ → **완료** (EMA 끔/약/강, 위 [완료된 항목](#완료된-항목) 6번 참고).
> ~~스포이드~~ → **완료** (`ToolKind.Eyedropper` + 합성 비트맵 픽셀 샘플, 위 [완료된 항목](#완료된-항목) 7번 참고).
> ~~최근 색 / 색 팔레트 저장~~ → **완료** (`UserPaletteRepo.addRecent`, 위 [완료된 항목](#완료된-항목) 8번 참고).

## P2 — 그다음 (정체성·재미, 약간 더 노력)

### 트레이싱 보조 (사진 따라 그리기) — 반투명 **완료**, 엣지 검출 P3 잔여
> 반투명(표시 알파) 완료 — 위 [완료된 항목](#완료된-항목) 9번. 아래는 엣지 검출(P3) 설계 메모.
- **개념**: 사진 배경을 반투명화하거나 외곽선만 추출해 오버레이 → 그 위를 따라 그림.
  **"사진 위 그리기" 라는 앱 정체성을 가장 직접적으로 강화.**
- **"사진 같이 저장"과는 직교(별개 축)**:
  - **트레이싱 보조 = *그리는 동안 화면에서* 사진을 얼마나 진하게 보여줄지(표시 알파).** 디스플레이 가이드.
  - **사진 배경 포함(`mergeBackgroundOnSave`) = *저장 결과물*에 사진을 구울지.** 출력(PNG).
  - 둘은 충돌하지 않고 같이 쓰임 — 전형적 워크플로: 사진을 **연하게 표시**(보조)해 따라 그린 뒤
    저장은 **사진 없이**(merge off) 해서 *내 선화만* 얻기. → 표시 알파라는 **새 로컬 상태 1개**만
    추가, 기존 저장 토글은 그대로.
- **구현**: 반투명은 캔버스 배경 그리기에 알파만 적용(`CanvasState` 에 표시 알파 상태 추가, 또는
  `DrawingViewModel` 로컬 상태 — 저장/동기화엔 미반영). 매우 가벼움 — **이 부분이 P2**. 외곽선 추출은
  엣지 검출(Sobel/Canny 근사) 후 라인 오버레이로 노력이 커 **엣지 부분은 P3**. 로컬 전용, 동기화 무관.
- **UI 위치**: 사진 관련 컨트롤이므로 **TopAppBar**(사진/촬영/제거/저장 옆), **사진 있을 때만 노출**
  (제거 버튼과 동일 패턴). 표시 알파 토글/순환(예: 진하게→연하게→아주 연하게).
- **난이도**: 반투명 낮음 / 엣지 검출 중. 반투명부터 단계적 도입 권장.
...
### 도형 채우기 토글
- **개념**: 현재 도형은 외곽선만 → 채움 옵션. 작지만 자주 쓰는 옵션.
- **구현**: `ToolSettings` 에 fill 플래그 추가, `StrokeRenderer` 도형 분기에서 채움 적용 →
  화면·PNG·동기화 자동. **난이도**: 낮음.

### 배경색 선택
- **개념**: 흰 배경 대신 파스텔 등 선택. 사진 없을 때 분위기.
- **구현**: 캔버스 배경색 상태 추가(현재 배경은 사진 슬롯뿐). 간단하지만 새 상태/저장 반영
  필요. **난이도**: 낮음.
...
(완료) 안내선 → 에어브러시 → 번짐 → 스티커 → 브러시 변형 4종(네온/점선/무지개/붓펜) → 손떨림 보정 → 스포이드 → 최근 색 **(P1 전부 완료)** → 트레이싱 반투명(P2).

이제 위 **P2 → P3 → P4** 티어 순서가 곧 권장 진행 순서. 요약:
- **P1**: ✅ 전부 완료
- **P2**: ~~트레이싱(반투명)~~✅ → 대칭 → 도형 채우기 → 배경색 (정체성·재미·작은 구조)
