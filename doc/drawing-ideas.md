# 그리기 모드 강화 아이디어

> 그리기 경험을 풍부하게 만들기 위한 기능 후보 목록. 사용자 제안 + 구현 관점 평가.
> 상태: 안내선부터 착수 예정, 나머지는 백로그.

## 평가 기준 (이 앱 특유)

- **동기화**: 모임/함께 모드에서 모든 그리기 요소는 `DrawingEvent` → `CanvasState.apply()`
  경로로 동기화돼야 함. 새 요소가 stroke 틀에 맞으면 거의 공짜, 아니면 와이어(`Frame`)
  까지 새로 만들어야 함.
- **렌더 공유**: `ui/canvas/StrokeRenderer.kt#drawStroke` 가 화면 + PNG 합성 공유. 여기에
  그리면 "보이는 것 = 저장되는 것" + 멀티 동기화가 자동.
- **정규화 좌표**: 모든 점은 0..1. 렌더 시 `canvasSize` 곱함.

## 후보

### 1. 상하 안내선 (가이드라인) — 착수 예정
- **개념**: 캔버스 위 보조선(수평/수직, 격자 등). 따라 그리기·대칭·구도 보조.
- **동기화**: 불필요 — stroke 아님, 순수 로컬 UI 오버레이 토글. 저장(PNG)에도 미포함.
- **난이도**: 낮음. `DrawingCanvas` 에 오버레이 레이어 + 도구바 토글.
- **결정 필요**: 안내선 종류(수평/수직/격자/3분할), 색/투명도, 토글 위치.

### 2. 에어브러시 (분사)
- **개념**: 점을 흩뿌리는 스프레이 브러시.
- **동기화 함정**: 난수 분사를 양쪽이 따로 굴리면 화면 불일치. **분사된 점 좌표를
  stroke points 에 실어 보내거나** seed 공유 필요.
- **난이도**: 중간. `BrushType` 추가 + StrokeRenderer 분기. 동기화는 좌표를 점으로
  박으면 기존 경로 재사용.

### 3. 번지는 효과 (수채/스머지)
- **개념**: 가장자리가 번지는 붓. `BlurMaskFilter` / alpha gradient.
- **동기화**: `BrushType` 추가라 자동.
- **주의**: blur 렌더 비용 큼 → 빽빽한 캔버스에서 프레임 드랍. 완료 stroke 비트맵
  캐시(roadmap Phase 5 보류 항목)가 더 절실해짐.
- **난이도**: 중간.

### 4. 스티커
- **개념**: 이미지/도형을 캔버스에 배치, 이동·크기·회전.
- **동기화**: stroke 아닌 **새 요소 타입** → `DrawingEvent`·`Frame`·`CanvasState`·
  렌더러·PNG 합성·undo 새 경로 전부 필요. 에셋도.
- **난이도**: 높음. 별도 미니 프로젝트로 분리 권장.

## 권장 진행 순서

안내선(빠른 win) → 에어브러시 → 번짐 → 스티커.
앞 셋은 기존 brush/오버레이 틀 재사용으로 가볍고, 스티커는 규모가 커서 마지막.
