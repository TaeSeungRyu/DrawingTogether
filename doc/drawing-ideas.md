...
7. **스포이드(색 추출)** — `ToolKind.Eyedropper` + 색 팔레트의 스포이드 버튼. 탭 시
   `PngComposer` 로 사진+stroke+스티커 합성 비트맵을 만들어 그 픽셀을 읽음("보이는 색=집히는 색").
...
(완료) 안내선 → 에어브러시 → 번짐 → 스티커 → 브러시 변형 4종(네온/점선/무지개/붓펜) → 손떨림 보정 → 스포이드.

이제 위 **P1 → P2 → P3 → P4** 티어 순서가 곧 권장 진행 순서. 요약:
- **P1**: 최근 색/팔레트 저장 (가장 가벼움)
