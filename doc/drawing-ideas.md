...
> 상태: 아래 "완료된 항목" 9건 완료(P1 전부 + 트레이싱 반투명). 나머지는 백로그(P2~P4).
...
> 트레이싱(반투명)은 완료(위 [완료된 항목](#완료된-항목) 9번). 엣지 검출 버전은 P3.
...
완료: 안내선 → 에어브러시 → 번짐 → 스티커 → 브러시 변형 4종 → 손떨림 보정 → 스포이드 → 최근 색 → 트레이싱 반투명.

남은 순서 (P2 → P3 → P4):
- **P2**: 대칭 → 도형 채우기 → 배경색 (정체성·재미·작은 구조)
