...
`DrawingTogether`는 두 모드를 같은 UI로 지원합니다.

- **싱글 모드**: 한 기기에서 혼자 그림. 네트워크 비활성. 사진 배경(선택/촬영)도 지원.
- **멀티 모드 (1:1)**: Nearby Connections로 연결된 두 기기 사이에서 드로잉 이벤트를 실시간 동기화. 한쪽이 사진을 보내면 양쪽 모두 같은 사진 위에 그림.

핵심 결정: **두 모드의 차이는 "이벤트가 어디서 오는가"뿐**. 로컬 입력이든 원격 인바운드든 동일한 `DrawingEvent` 스트림으로 정규화해 캔버스에 적용. 사진도 동일 — 자기가 고른 것이든 상대가 보낸 것이든 같은 `BackgroundImage` 상태에 들어감.
