...
### 방을 개설한 사람 (호스트 / 교사)
- 자기 그림 **+ 사진 배경**이 **모든 조인자에게 라이브로** 보인다(방장 라이브뷰).
- **"참여자 보기" 버튼**을 누르면 **모달(다이얼로그)**이 열린다.
- 모달에서 참여한 인원의 **이름을 선택**하면 그 인원이 **지금 무엇을 그리는지 라이브로** 본다(한 번에 한 명, 온디맨드). 조인자의 사진 배경도 함께 보인다.

### 참여한 사람 (조인자 / 학생)
- 방을 개설한 사람(호스트)의 내용(**그림 + 사진**)을 **항상 라이브로 본다**(보조 뷰). 보조 뷰가 작으므로 **탭하면 큰 모달**로 본다.
- 호스트의 내용을 자기 캔버스로 **가져올 수 있다**("가져오기" = 스냅샷 pull). 도구바 동기화 버튼은 없음.
- **다른 참여 인원의 내용(이름·그림)은 볼 수 없다.**

> 사진 공유(역할 무관): 라이브뷰엔 그 사람의 사진 배경도 함께 표시된다(호스트↔조인자 양방향).
> 단 조인자→조인자는 안 보임(호스트가 relay 안 함). 미니/모달 뷰는 그 사진의 가로세로 비율을 따른다.
