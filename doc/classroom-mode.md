...
### 참여한 사람 (조인자 / 학생)
- 방을 개설한 사람(호스트)의 내용을 **항상 라이브로 본다**(보조 뷰).
- 호스트의 내용을 자기 캔버스로 **가져올 수 있다**("가져오기" = 스냅샷 pull).
...
1. **조인자 호스트뷰 초기 채움** — 라이브는 합류 후 발생분만. 합류 시점 호스트 현재 내용은 "가져오기"로 확인(자동 초기 스냅샷 송신은 후속).
2. **호스트 사진 배경 라이브 표시** — 사진은 자동 broadcast 안 함. 라이브 호스트뷰엔 stroke/스티커/텍스트만, 사진은 "가져오기" 시 동반(라이브 사진 스트리밍은 후속).
3. **pull 후 호스트의 조인자뷰 staleness** — 조인자가 호스트 내용을 pull(applySnapshot은 이벤트 미발화)하면, 호스트가 그 조인자를 볼 때 pull 직전 상태로 보일 수 있음(후속 보정).
